...
def astar(this_graph: Graph, start: int) -> dict | None:
    """
     ...
     Пока в куче есть вершины, извлекается вершина с наименьшей оценкой длины пути.
     Если текущая вершина равна целевой, то строится путь от начальной до конечной
     вершины,используя информацию из came_from.
     Для каждого соседа текущей вершины вычисляется временная оценка длины пути.
